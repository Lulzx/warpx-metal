...
Benchmark results are in `benchmarks/RESULTS.md` (auto-generated by `08-benchmark.sh`).
The test matrix covers 2D (128x128, 512x512) and 3D (64^3, 128^3) Langmuir wave
simulations. CPU baseline uses 12 OpenMP threads (all M4 Pro cores); GPU uses Metal
via AdaptiveCpp SSCP. GPU times exclude first-run JIT compilation.
...
Key profiling findings (warm JIT, 512x512, 4 ppc):

| Kernel | ms/step | Root cause |
|--------|---------|------------|
| FillBoundary_nowait | 62ms | 15 Metal dispatches/step × 4ms IOKit overhead each |
| ParticleCopyPlan | 34ms | GPU prefix scan via ~20 Metal dispatches |
| Redistribute | 20ms | Same dispatch overhead |
| GatherAndPush | 12ms | Actual particle physics — tied with CPU |
| FieldSolve (B+E) | 11ms | Actual field physics |

71% of wall time is `sys` (Metal command buffer submission). GPU physics compute
(GatherAndPush + FieldSolve + Deposition = 31ms/step) is already comparable to CPU,
but 138ms/step of Metal dispatch overhead dominates at these problem sizes.
